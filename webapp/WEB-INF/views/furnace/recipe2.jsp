...
<script>
  function validateNumberInput(event) {
	...
	var input_data = event.data;
	var input_id = input.id;
      
	if(input_id.match("process_step")){
		var idArray = input_id.split("_");				
		var idx = eval(idArray[2]);
			
			if(input_data == "0"){
				for(var i=idx+1; i<40; i++){
					$("#process_step_"+i).attr("disabled", true);
					$("#process_step_"+i).val(0);
					$("#time_"+i).attr("disabled", true);
					$("#time_"+i).val(0);
					$("#temperature_"+i).attr("disabled", true);
					$("#temperature_"+i).val(0);
				}				
			}else{
				for(var i=idx; i<40; i++){
					$("#process_step_"+i).removeAttr("disabled");
					$("#time_"+i).removeAttr("disabled");
					$("#temperature_"+i).removeAttr("disabled");
				}
			}
		}
	}
